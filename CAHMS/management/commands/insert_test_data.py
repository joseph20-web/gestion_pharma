...
class Command(BaseCommand):
    help = 'Insérer des données de test (10 fournisseurs et 100 produits)'
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('🚀 Début de l\'insertion des données de test...')
        )
        
        # Créer les fournisseurs
        self.stdout.write('\n📦 Création des fournisseurs...')
        fournisseurs = self.create_fournisseurs()
        self.stdout.write(
            self.style.SUCCESS(f'✅ {len(fournisseurs)} fournisseurs traités')
        )
        
        # Créer les produits
        self.stdout.write('\n💊 Création des produits...')
        produits = self.create_products()
        self.stdout.write(
            self.style.SUCCESS(f'✅ {len(produits)} produits traités')
        )
        
        self.stdout.write('\n' + '=' * 50)
        self.stdout.write(
            self.style.SUCCESS('🎉 Insertion des données terminée avec succès!')
        )
    ...
    def create_fournisseurs(self):
        ...
        for designation in fournisseurs_data:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f'✅ Fournisseur créé: {designation}')
                )
            else:
                self.stdout.write(
                    self.style.WARNING(f'⚠️ Fournisseur existant: {designation}')
                )
    ...
    def create_products(self):
        ...
        for i in range(100):
            ...
            if created:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'✅ Produit créé: {produit.nom_produit} - {prix} FC - Stock: {produit.quantite_stock}')
                )
            else:
                self.stdout.write(
                    self.style.WARNING(f'⚠️ Produit existant: {produit.nom_produit}')
                )
